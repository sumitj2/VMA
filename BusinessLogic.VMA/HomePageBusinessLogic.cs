    ...
    public class HomePageBusinessLogic : IHomePageBusinessLogic
    {
        ...
        public async Task<DashboardDetailsModel> GetDashboardDetails(string? financialYear)
        {
            ...
            if (result != null)
            {
                ...
                dashboardDetailsModel.SantionedAmtPiad = result.SantionedAmtPaid;
                dashboardDetailsModel.TotalSanctionAmount = result.TotalSanctionAmount;
                dashboardDetailsModel.OtherAmtPaid = result.OtherAmtPaid;
            }
        ...
        public async Task<List<YearlyReportDataModel>> GetDashboardServicesBarChartDetails(string? financialYear)
        {
            ...
            foreach (var yearRepor in result)
            {
                ...
                {
                    NumberOfTerms = yearRepor.ServicePaymentType == GeneralConstants.PaymentTypeNone || yearRepor.ServicePaymentType == GeneralConstants.PaymentTypeNone ? yearRepor.TotalPaymentsMade : yearRepor.NumberOfTerms,
                    RemainingAmount = yearRepor.RemainingAmount,
                    RemainingTerms = yearRepor.ServicePaymentType==GeneralConstants.PaymentTypeNone || yearRepor.ServicePaymentType == GeneralConstants.PaymentTypeNoneWithSantionedAmt ? 0:yearRepor.RemainingTerms,
